...
then
      ...
      mvn clean package -PrestartIndexWithSolrRunning -DmitabFile=${MITAB_FILE} -DsolrUrl=${SOLR_URL} -Dindexing.id=${INDEXING_ID} -Dmaven.test.skip
elif [ $# == 2 ];
then
      MITAB_FILE=$1;
      INDEXING_ID=$2;
      ...
      echo "Indexing job ID: ${INDEXING_ID}"
      mvn clean package -PrestartIndexWithSolrRunning -DmitabFile=${MITAB_FILE} -DsolrUrl="http://localhost:9090/solr/" -Dindexing.id=${INDEXING_ID} -Dmaven.test.skip
else
      echo ""
