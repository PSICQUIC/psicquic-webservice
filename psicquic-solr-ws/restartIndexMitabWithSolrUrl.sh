...
then
      ...
      mvn install -PrestartIndexWithSolrRunning -Dmitab.file=${MITAB_FILE} -Dsolr.url=${SOLR_URL} -Dindexing.id=${INDEXING_ID} -Dmaven.test.skip
elif [ $# == 2 ];
then
      MITAB_FILE=$1;
      INDEXING_ID=$2;
      ...
      echo "Indexing job ID: ${INDEXING_ID}"
      mvn install -PrestartIndexWithSolrRunning -Dmitab.file=${MITAB_FILE} -Dindexing.id=${INDEXING_ID} -Dmaven.test.skip
else
      echo ""
