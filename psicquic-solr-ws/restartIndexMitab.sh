...
then
      ...
      mvn clean install -PrestartIndex -DmitabFile=${MITAB_FILE} -Dsolr.workdir=${SOLR_WORKDIR} -Dindexing.id=${INDEXING_ID}
elif [ $# == 2 ];
then
      MITAB_FILE=$1;
      INDEXING_ID=$2;
      ...
      echo "Indexing job ID: ${INDEXING_ID}"
      mvn clean install -PrestartIndex -DmitabFile=${MITAB_FILE} -Dindexing.id=${INDEXING_ID}
else
      echo ""
